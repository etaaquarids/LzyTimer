...
namespace Lzy::Timer {
	...
	template<
		...
		typename executor = Lzy::Timer::std_async
	>
	struct Timer {
	...
	public:
		void start() {
			std::thread([this]() {
				while (true) {
					Lzy::sleep_for(tick_once() - Clock::now());
				}
			}).detach();
		}

		void setTimeout(std::invocable auto&& fn, const chrono_time auto& duration) {
		...
		Clock::time_point tick_once() {
			static auto last_tick_time = Clock::now();
			last_tick_time += tick_time;
			time_wheels.tick_once([](const auto& fn) { executor::execute(fn); });
